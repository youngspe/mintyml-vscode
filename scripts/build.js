...
async function convertTmlanguage() {
  /** @type { JsonObject } */
  const doc = yaml.load(await fs.readFile(TM_SRC, { encoding: 'utf8' }));
  ...
  const writePromise = fs.writeFile(TM_DST, JSON.stringify(doc, null, 2), { encoding: 'utf8' });
...
async function convertLanguageConfig() {
  /** @type { JsonObject } */
  const doc = yaml.load(await fs.readFile(LANG_CONFIG_SRC, { encoding: 'utf8' }));

  await fs.writeFile(LANG_CONFIG_DST, JSON.stringify(doc, null, 2));
}

await Promise.all([convertTmlanguage(), convertLanguageConfig()]);
